...
name_email_login() {
        ...
        if [ ! -z "$username" ] && [ ! -z "$password" ]; then
                declare -A users
                users=( ["guest1"]="password1" ["guest2"]="password2")
...
login_form() {
        ...
        echo "
                ...
                <med-blue>You are connected to CORP Guest WiFi network</med-blue><br>
                <italic-black>
                        To access the Internet you must enter your username and your password then Accept the Terms of Service to proceed.
                </italic-black>
                <hr>
...
thankyou_page () {
        # If we got here, we have both the username and password fields as completed on the login page on the client,
...
landing_page() {
        ...
        userinfo="$userinfo, user=$username, password=$password"
